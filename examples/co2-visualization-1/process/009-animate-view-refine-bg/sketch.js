/**
 * step 9: refine the background color for effect
 */
...
function setup() {
	...
	// 8.5 slow down this sketch to make it more watchable
	frameRate(10)
}
...
function draw() {
	...
	// 9.2 set the background saturation to increase as the counter increases
	background(1, co2Val, 100);

		// 8.1 display header and subheader
  ...
  text('1958 - 2018', width / 2, height / 8);
	...
	// 8.2 add the stroke back in
	stroke(0);
	ellipse(width/2, height/2, co2Val, co2Val);

	// 8.3 add in the month and the year
	// Yr Mn
	text(myCo2DataFiltered[counter].Mn + " / " + myCo2DataFiltered[counter].Yr, width / 2, height - 20);

	// 8.4 add in the co2 values
	text( myCo2DataFiltered[counter].CO2_filled_ppm, width / 2, height/2);
