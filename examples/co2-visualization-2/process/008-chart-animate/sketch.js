...
// 8.1 create a counter for the animation
var counter = 0;
...
function draw() {
	// 8.4 change the background alpha to get the animated ghosting effect
	...
	// 8.2 animate the chart with the counter variable
	line(myCo2DataFiltered[counter].screen_x, myCo2DataFiltered[counter].screen_y, myCo2DataFiltered[counter+1].screen_x, myCo2DataFiltered[counter+1].screen_y)
	...
	// 8.3 increment the counter until the length of the data then start over again
	if(counter < myCo2DataFiltered.length-2){
		counter++;
	} else{
		counter = 0;
	}
